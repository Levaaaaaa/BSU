...
#        raise NotImplementedError("COMPLETE THIS FUNCTION")
        if hasattr(estimator, "predict_proba"):  # choose the most informative method for stacking predictions
            ...
            method = "predict"
        cross_val_stacking_prediction = sklearn.model_selection.cross_val_predict(estimator=estimator, X=self.X_train, y=self.y_train, cv=self._cv_iterable, method=method)  
